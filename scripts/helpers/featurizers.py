...
class PerovskiteProperty(BaseFeaturizer):
	...
	"""
	def __init__(self, cation_site, site_ox_lim, site_base_ox):
	...
	def set_featurize_options(self,sites,ox_stats=['min','max','mean','median','std','range'],ep_stats=["mean", "std_dev", "range"],radius_type='ionic_radius',normalize_formula=True,silent=True):
		"""
		Set options for featurization. Since these options change the number and meaning of features returned, safest to set for the whole class for consistency, 
		rather than passing as inputs to each featurize() call
	...
	def featurize(self,formula):
		...
		"""
		try:
			pvsk = Perovskite(formula, self.cation_site, self.site_ox_lim, self.site_base_ox,self.featurize_options['radius_type'],self.featurize_options['normalize_formula'],self.featurize_options['silent'])
		...
		for site in self.featurize_options['sites']:
			...
			ao_features = [ao_features[i] for i in range(len(ao_features)) if i not in (1,4)]#exclude HOMO_element and LUMO_element - categoricals
